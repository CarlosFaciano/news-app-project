...
export default function DetailedNews() {
  let { title, query } = useParams()
  title = decodeURIComponent(title)
  const [specific, setSpecific] = useState()

  const url = `https://newsapi.org/v2/top-headlines?q=${query}&apiKey=716cffddaf1946a199d81f0d5147b014`
  ...
  return (
    <div className='h-screen w-screen flex flex-col bg-slate-950 overflow-x-hidden items-center overflow-y-auto'>
      ...
      <button className="mt-5 bg-sky-800 hover:duration-300 hover:ease-in-out hover:bg-sky-900 rounded-xl items-center justify-center h-9 w-36 py-2 text-white">
      ...
      <div className='text-start w-full flex items-center m-4 text-white'>
        <Link className='flex gap-x-2 ms-4' href="/">
          <ArrowLeft />
          Go back to home
        </Link>
      </div>
